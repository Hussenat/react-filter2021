...
test("should render ExpenseForm correctly", ()=>{
    ...
    //- the 1st time we run jest suite its will work fine but when we rerun the jest suite its will fail beco's of the date props pass onto <singleDatePicker/> component will keep on changing everytime we run jest test suite
    //- to solve the above issue we need to create mock of moment instance which is a fake moment instance which will always return a moment at a particular point in time.
    //- create folder called __mocks__ inside src/tests folder.
    //- create a file called __mocks__/moment.js and set it up
    //- rerun jest test suite then press u to update the snapshot file
...
test("should set note on textarea", ()=>{
    const value = "Brought new television";
    const wrapper = shallow(<ExpenseForm />);
...
// the below block of code is what we are testing which is from ExpenseForm in order to dispatch on AddExpensePage.
// else{
//     this.setState(()=>({error:""}))
...
test("should set new date on date change", ()=>{
    const now = moment(); //moment is a value store on createdAt property
    const wrapper = shallow(<ExpenseForm />)
...
test("should set calender focus on change", ()=>{
    ...
    expect(wrapper.state("calenderFocused")).toBe(focused) //singleDatePicker is a tag
})